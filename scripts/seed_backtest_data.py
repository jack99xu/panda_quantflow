...
def main():
    ...
    # 3. 检查是否已有数据（幂等：有数据则跳过）
    if db.stock_market.count_documents({}) > 0:
        count = db.stock_market.count_documents({})
        print(f"\n✅ 行情数据已存在（{count} 条记录），跳过预加载")
        client.close()
        return
    ...
    try:
        ...
        # ---- 股票基本信息 ----
        print("\n[3/4] 生成股票信息 (stock_info_new)...")
        info_docs = []
        for code, name in STOCK_LIST:
            symbol = get_symbol(code)
            # type: 0=普通股票, 但实际按源码走只要不是1(index)/2(ETF)就行
            info_docs.append({
                "symbol": symbol,
                "name": name,
                "type": 0,
        ...
        if info_docs:
            ...
            print(f"   √ {len(info_docs)} 只股票")

        # ---- 股票日线行情 ----
        print("\n[4/4] 下载股票日线数据 (stock_market)...")
        total_records = 0
        for code, name in STOCK_LIST:
            symbol = get_symbol(code)
            try:
        ...
        print(f"\n{'=' * 60}")
        print(f"✅  数据预加载完成！共导入 {total_records} 条行情记录")
        print(f"   股票: {len(STOCK_LIST)} 只, 日期: {START_DATE} ~ {END_DATE}")
        print(f"{'=' * 60}")
